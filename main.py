...
class JarvisV4:
    def __init__(self) -> None:
        ...
        self.wake_word_min_listen_seconds = float(CONFIG.get("wake_word_min_listen_seconds", 0.90))
        self.wake_word_end_silence_seconds = float(CONFIG.get("wake_word_end_silence_seconds", 1.10))
        self.wake_word_energy_threshold = float(CONFIG.get("wake_word_energy_threshold", 550.0))

        self.visuals = VisualStateController()
        self.speaker = Speaker(samplerate=self.sample_rate_out, blocksize=self.speaker_blocksize, state_callback=self.visuals.set_state)
        ...
        self.ptt = PushToTalkController(on_activated=self._on_ptt_activated, on_released=self._on_ptt_released)
    ...
    async def startup(self) -> None:
        ...
        notify(f"Jarvis v4 started. {status}")
        log_event("jarvis_started", {"wake_word_ready": self.wakeword.is_ready, "wake_word_error": self.wakeword.error})
    ...
    def _on_ptt_activated(self) -> None:
        if self.loop:
            self.loop.call_soon_threadsafe(lambda: asyncio.create_task(self._begin_turn("ptt")))

    def _on_ptt_released(self) -> None:
        if self.loop:
            self.loop.call_soon_threadsafe(lambda: asyncio.create_task(self._commit_turn("ptt_release")))
    ...
    async def _handle_active_turn(self, chunk: np.ndarray) -> None:
        ...
        if self.user_turn_source == "wakeword":
            ...
            if elapsed >= self.wake_word_min_listen_seconds and silent_for >= self.wake_word_end_silence_seconds:
        ...
        try:
            while self.running:
                ...
                if self.user_turn_active:
                    ...
                    if not self.speaker.is_speaking and not self.client.busy and not self.user_turn_active:
                        self._set_idle_visual_state()
